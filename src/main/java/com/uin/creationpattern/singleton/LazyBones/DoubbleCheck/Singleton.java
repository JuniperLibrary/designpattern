...
public class Singleton {
  ...
  /**
   * 当一个变量定义为volatile之后，它将具备两种特性 1. 保证此变量对所有线程的可见性 这里不做过多解释，简单的说就是，当一个线程修改了volatile变量之后，它先写入它的工作内存中，
   * 然后立刻写入主内存，并且刷新其他线程中的工作内存，这样其他线程再去读取他们工作内存中的变量时，
   * 确保能够拿到最新的。但是如果是普通变量的话，它不会立即写入主内存中，所有其他线程的工作内存中保存的是旧的值。 所有volatile变量可以保证可见性。 2. 禁止指令重排序优化
   */
